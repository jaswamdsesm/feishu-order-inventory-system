...
async function init(){
  try{
    ...
    if(cached){try{
      ...
      currentUser=u; currentRole=u.role||'employee'; feishuUid=u.feishu_user_id||'';
      hideLoading(); applyRole();
      await Promise.all([loadProfiles(),loadProducts(),loadOrders(),loadInventoryLogs()]);
      switchPage('dashboard'); return;
    }catch(e){}}
    await feishuLogin();
  }catch(err){console.error(err);hideLoading();showToast('初始化失败:'+err.message,'error');}
}
...
async function feishuLogin(){
  ...
  if(!code){
    window.location.href=`https://open.feishu.cn/open-apis/authen/v1/authorize?app_id=${FEISHU_APP_ID}&redirect_uri=${encodeURIComponent(window.location.origin+window.location.pathname)}&response_type=code`;
    return;
  }
  try{
    ...
    currentUser=data.user; currentRole=data.user.role||'employee'; feishuUid=data.user.feishu_user_id||'';
...
function feishuLogout(){localStorage.removeItem('oi_user');window.location.href=window.location.pathname;}
...
function applyRole(){
  ...
  document.getElementById('sidebar-user-role').textContent=roleText[currentRole]||currentRole;
}

// ============ 侧边栏/夜间模式/页面切换 ============
function toggleSidebar(){const sb=document.getElementById('sidebar'),ol=document.getElementById('sidebar-overlay'),open=!sb.classList.contains('-translate-x-full');sb.classList.toggle('-translate-x-full',open);ol.classList.toggle('hidden',open);}
(function(){const s=localStorage.getItem('oi_dark_mode');if(s==='true'||(!s&&window.matchMedia('(prefers-color-scheme: dark)').matches))document.documentElement.classList.add('dark');updateDarkModeUI();window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change',e=>{if(!localStorage.getItem('oi_dark_mode')){document.documentElement.classList.toggle('dark',e.matches);updateDarkModeUI();}});})();
function toggleDarkMode(){const d=document.documentElement.classList.toggle('dark');localStorage.setItem('oi_dark_mode',d);updateDarkModeUI();}
function updateDarkModeUI(){const d=document.documentElement.classList.contains('dark');const i=document.getElementById('dark-mode-icon');const t=document.getElementById('dark-mode-text');if(i)i.textContent=d?'☀️':'🌙';if(t)t.textContent=d?'日间模式':'夜间模式';}
...
function switchPage(page){
  ...
  const te=document.getElementById('page-title');if(te)te.textContent=titles[page]||'';
...
function showToast(msg,type='info'){const c={success:'bg-green-500',error:'bg-red-500',warning:'bg-yellow-500',info:'bg-blue-500'};const t=document.createElement('div');t.className=`fixed top-20 left-1/2 -translate-x-1/2 px-5 py-3 rounded-xl text-white text-sm font-medium shadow-lg z-[100] ${c[type]||c.info}`;t.textContent=msg;document.body.appendChild(t);setTimeout(()=>{t.style.opacity='0';t.style.transition='opacity .4s';setTimeout(()=>t.remove(),400);},2500);}
...
function genOrderNo(){const d=new Date(),ds=d.getFullYear()+String(d.getMonth()+1).padStart(2,'0')+String(d.getDate()).padStart(2,'0');return 'ORD'+ds+String(Math.floor(Math.random()*10000)).padStart(4,'0');}
function isPhoneHidden(){return currentRole==='employee';}

// ============ 数据加载 ============
async function loadProfiles(){const{r,error}=await sb.from('profiles').select('*');if(!error)allProfiles=r||[];}
async function loadProducts(){const{r,error}=await sb.from('products').select('*').order('name');if(!error)allProducts=r||[];return allProducts;}
async function loadOrders(){const{r,error}=await sb.from('orders').select('*').order('created_at',{ascending:false});if(!error)allOrders=r||[];const{data:items,error:e2}=await sb.from('order_items').select('*');if(!e2)allOrderItems=items||[];return allOrders;}
async function loadInventoryLogs(){const{r,error}=await sb.from('inventory_logs').select('*').order('created_at',{ascending:false}).limit(200);if(!error)allInventoryLogs=r||[];}
...
function openProductModal(id){
  ...
  if(id){
    ...
    if(p){document.getElementById('product-name').value=p.name||'';document.getElementById('product-sku').value=p.sku||'';document.getElementById('product-stock').value=p.current_stock||0;document.getElementById('product-alert').value=p.min_stock_alert||10;document.getElementById('product-unit').value=p.unit||'个';}
  }else{
    document.getElementById('product-name').value='';document.getElementById('product-sku').value='';document.getElementById('product-stock').value=0;document.getElementById('product-alert').value=10;document.getElementById('product-unit').value='个';
  }
...
async function saveProduct(){
  ...
  const btn=document.getElementById('btn-save-product');btn.disabled=true;btn.textContent='保存中…';
  try{
    ...
    closeModal('modal-product');await loadProducts();renderInventory();showToast(id?'产品已更新':'产品已添加','success');
...
async function deleteProduct(id){
  ...
  try{const{error}=await sb.rpc('delete_product',{p_id:id,p_feishu_user_id:feishuUid});if(error)throw error;await loadProducts();renderInventory();showToast('产品已删除','success');}
...
function addOrderItemRow(existing){
  ...
  if(existing&&existing.product_id)setTimeout(()=>{document.getElementById('item-product-'+idx).value=existing.product_id;},0);
...
async function saveOrder(){
  ...
  try{
    ...
    if(error)throw error;
    closeModal('modal-order');await Promise.all([loadProducts(),loadOrders(),loadInventoryLogs()]);renderOrders();renderInventory();showToast(isEdit?'订单已更新':'订单已创建','success');
  }catch(err){showToast('保存失败:'+err.message,'error');}
...
async function deleteOrder(id){
  ...
  try{
    ...
    if(error)throw error;
    await Promise.all([loadProducts(),loadOrders(),loadInventoryLogs()]);renderOrders();renderInventory();showToast('订单已删除，库存已回滚','success');
  }catch(err){showToast('删除失败:'+err.message,'error');}
}
...
async function handleBatchOrderPaste(){
  ...
  if(results.length>0&&confirm(`确认导入 ${results.length} 条订单？`)){
    await batchImportOrders(results);
  }
}
...
async function batchImportOrders(results){
  ...
  closeModal('modal-batch-order');await Promise.all([loadProducts(),loadOrders(),loadInventoryLogs()]);renderOrders();renderInventory();showToast(`导入完成：成功 ${success} 条`+(fail>0?`，失败 ${fail} 条`:''),success>0?'success':'error');
}
...
async function changeUserRole(userId,newRole){
  ...
  try{
    ...
    if(error)throw error;
    await loadProfiles();renderUsers();showToast('角色已更新','success');
